...
def register():
    #Function to user register if current user is authenticated return page tasks 
    if current_user.is_authenticated:
        return redirect(url_for('.tasks'))
...
def get_task(task_id):
    #Function to see only the task of each user based on the id
...
def edit_task(task_id):
    #Function to edit only the task of each user based on the id
    task = Task.query.get_or_404(task_id)
